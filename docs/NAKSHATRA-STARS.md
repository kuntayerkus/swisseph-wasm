...
Implementation deliberately not yet merged — see the integration note at the
end. This document exists to answer the roadmap item: *"getting them right
needs a proper source"*.
...
## Integration note (next phase)

When this becomes code, the shape should mirror the existing star curation
(`packages/core/src/derived/stars.ts`, catalogue in
`packages/core/src/generated/stars.ts`): a `nakshatra` group on
`CuratedStar` entries, looked up via `byDesignation()`, so the positions
come from `sefstars.txt` at runtime and never from a hand-typed table. The
junction-star choice belongs in metadata with its `source` field pointing at
this document. The consumer is `@kuntay/swisseph-advanced`
(`packages/advanced/src/Nakshatra.ts`), which currently carries the 27
nakshatra ranges but no stars.
