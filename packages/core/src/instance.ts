...
import {
  ASCMC, Body, Calendar, EclipseFlag, EclipticNutationId, Flag, HouseSystem,
  RiseTransit, SIGNS,
} from './constants.js';
...
import {
  calculateLots, type ChartPoints, type LotDefinition, type LotResult,
} from './derived/lots.js';
import type { DeclinationPoint } from './derived/declination.js';
import {
  ANGLE_EVENTS, findParans,
  type AngleEvent, type AngleEventTimes, type ParanContact, type ParanOptions,
} from './derived/parans.js';
import {
  lunarEclipseKind, optionalTime, solarEclipseKind,
  type LunarEclipse, type SolarEclipse,
} from './derived/eclipses.js';
import {
  atmosphereToArray, observerToArray,
  type Atmosphere, type HeliacalResult, type Observer,
 ...
 */
const isGauquelin = (system: string) => system.charAt(0).toUpperCase() === 'G';
 ...
 */
const MEAN_RETURN_DAYS: Record<number, number> = {
...
const EPHEMERIS_FLAG: Record<EphemerisModel, number> = {
 ...
 * arama yolu.
 */
function packageProviding(file: string): string | null {
  ...
  if (/^s\d/.test(file) || /^se\d/.test(file)) return '@kuntay/swisseph-asteroids';
  return null;
}
 ...
 */
function ephemerisError(serr: string | null, fn: string): SwissEphError {
  ...
  return new SwissEphError(
    `Ephemeris file '${file}' is not loaded. ` +
    (pkg ? `It ships in ${pkg}. ` : '') +
    ...
    'is why this raised instead of degrading.',
    fn,
    serr,
  );
}
...
function ephemerisFromFlags(returnedFlags: number): EphemerisModel {
  if (returnedFlags & Flag.Moshier) return 'moshier';
  if (returnedFlags & Flag.JplEphemeris) return 'jpl';
  return 'swiss';
}
...
export async function createSwissEph(options: SwissEphOptions = {}) {
  ...
  const c = {
    ...
    azalt: wasm.cwrap('swe_azalt', null,
      ['number', 'number', 'number', 'number', 'number', 'number', 'number']),
    riseTrans: wasm.cwrap('swe_rise_trans', 'number',
      ['number', 'number', 'number', 'number', 'number',
       'number', 'number', 'number', 'number', 'number']),
    solcrossUt: wasm.cwrap('swe_solcross_ut', 'number',
      ['number', 'number', 'number', 'number']),
    mooncrossUt: wasm.cwrap('swe_mooncross_ut', 'number',
      ['number', 'number', 'number', 'number']),
    solEclipseWhenGlob: wasm.cwrap('swe_sol_eclipse_when_glob', 'number',
      ['number', 'number', 'number', 'number', 'number', 'number']),
    solEclipseWhenLoc: wasm.cwrap('swe_sol_eclipse_when_loc', 'number',
      ['number', 'number', 'number', 'number', 'number', 'number', 'number']),
    lunEclipseWhen: wasm.cwrap('swe_lun_eclipse_when', 'number',
      ['number', 'number', 'number', 'number', 'number', 'number']),
    lunEclipseWhenLoc: wasm.cwrap('swe_lun_eclipse_when_loc', 'number',
      ['number', 'number', 'number', 'number', 'number', 'number', 'number']),
    heliacalUt: wasm.cwrap('swe_heliacal_ut', 'number',
      ['number', 'number', 'number', 'number', 'number',
       'number', 'number', 'number', 'number']),
  ...
  const readDoubles = (ptr: number, n: number): number[] =>
    Array.from({ length: n }, (_, i) => wasm.getValue(ptr + i * 8, 'double') as number);

  const readError = (): string | null => {
    const s = wasm.UTF8ToString(bufError);
    return s && s.length ? s : null;
  };

  const writeDoubles = (ptr: number, values: readonly number[]): void => {
  ...
  const zeroDoubles = (ptr: number, n: number): void => {
    for (let i = 0; i < n; i++) wasm.setValue(ptr + i * 8, 0, 'double');
  };

  const writeGeo = (place: GeoPosition): void => {
  ...
  function returnOf(
    ...
    natalJd: number,
    options: CalcOptions & {
      after?: number; precessionCorrected?: boolean; maxDays?: number;
    } = {},
  ): ReturnResult {
  ...
  if (options.files) mountFiles(options.files, options.ephemerisPath);
  else if (options.ephemerisPath) c.setEphePath(options.ephemerisPath);
  ...
  const api = {
    ...
    julianDay(
      ...
      hour = 0, calendar: number = Calendar.Gregorian,
    ): number {
      assertLive();
    ...
    calendarDate(jd: number, calendar: number = Calendar.Gregorian): CalendarDate {
      assertLive();
      ...
      return {
        year: wasm.getValue(y, 'i32') as number,
        month: wasm.getValue(m, 'i32') as number,
        day: wasm.getValue(d, 'i32') as number,
        hour: wasm.getValue(h, 'double') as number,
      };
    },

    /** ΔT (TT − UT) in days. */
    deltaT(jd: number, ephemeris: EphemerisModel = 'swiss'): number {
      assertLive();
    ...
    calc(jd: number, body: number, options: CalcOptions = {}): Position {
      ...
      const [longitude, latitude, distance,
             longitudeSpeed, latitudeSpeed, distanceSpeed] = readDoubles(bufPosition, 6);
    ...
    fixedStar(name: string, jd: number, options: CalcOptions = {}): StarPosition {
      ...
      if (ret < 0) {
        ...
        throw new SwissEphError(
          message ?? `Fixed star not found: ${name}`, 'swe_fixstar2');
      }
    ...
    houses(
      ...
      latitude: number,
      longitude: number,
      system: string = HouseSystem.Placidus,
      options: CalcOptions = {},
    ): Houses {
      assertLive();
      ...
      const ret = c.housesEx2(
        jd, flags, latitude, longitude, system.charCodeAt(0),
        bufCusps, bufAscmc, bufCuspSpeed, bufAscmcSpeed, bufError,
      );
      const message = readError();
    ...
    lots(
      ...
      sect: SectResult & { overridden: boolean };
      points: ChartPoints;
      ...
      const sectResult: SectResult & { overridden: boolean } = options.sect
    ...
    horizontal(
      ...
      altitudeMetres = 0,
      options: CalcOptions = {},
    ): { azimuth: number; altitude: number; apparentAltitude: number } {
      assertLive();
    ...
    sect(
      ...
      latitude: number,
      longitude: number,
      options: SectOptions & { houseSystem?: string } = {},
    ): SectResult {
      assertLive();
      ...
      if (options.method === 'ascendant') {
        const houses = api.houses(jd, latitude, longitude,
    ...
    obliquity(jd: number, options: CalcOptions = {}): Obliquity {
      ...
      const [trueObliquity, meanObliquity, nutationInLongitude, nutationInObliquity] =
        readDoubles(bufPosition, 4);
     ...
     */
    declinations(
      jd: number, bodies: readonly number[], options: CalcOptions = {},
    ): DeclinationPoint[] {
    ...
    riseTransit(
      ...
      place: GeoPosition,
      event: number = RiseTransit.Rise,
      options: CalcOptions & { pressure?: number; temperature?: number } = {},
    ): { jd: number | null; occurs: boolean } {
      assertLive();
      const isStar = typeof target === 'string';
      const flags = EPHEMERIS_FLAG[options.ephemeris ?? 'swiss'] | (options.flags ?? 0);

      if (isStar) wasm.stringToUTF8(target, bufObjectName, ERROR_BUFFER_SIZE);
      writeGeo(place);
      wasm.setValue(bufError, 0, 'i8');
      zeroDoubles(bufTret, 1);

      const ret = c.riseTrans(
        jd, isStar ? 0 : target, isStar ? bufObjectName : 0,
        flags, event, bufGeo,
        options.pressure ?? 0, options.temperature ?? 0,
        bufTret, bufError,
      );

      if (ret === RISE_TRANS_NO_EVENT) return { jd: null, occurs: false };
      if (ret < 0) {
        throw new SwissEphError(readError() ?? 'swe_rise_trans failed', 'swe_rise_trans');
      }
      return { jd: wasm.getValue(bufTret, 'double') as number, occurs: true };
    },
    ...
    angleEvents(
      jd: number,
      targets: readonly (number | string)[],
      place: GeoPosition,
      options: CalcOptions & { pressure?: number; temperature?: number } = {},
    ): AngleEventTimes[] {
      assertLive();
      const bits: Record<AngleEvent, number> = {
      ...
      return targets.map((target) => {
        const isStar = typeof target === 'string';
        const events: Partial<Record<AngleEvent, number>> = {};

        for (const event of ANGLE_EVENTS) {
          const r = api.riseTransit(jd, target, place, bits[event], options);
          if (r.occurs && r.jd !== null) events[event] = r.jd;
        }

        const entry: AngleEventTimes = {
        ...
        if (events.rise === undefined && events.set === undefined) {
          ...
          if (signed > limit) entry.circumpolar = true;
          else if (signed < -limit) entry.neverRises = true;
        }
    ...
    parans(
      ...
      targets: readonly (number | string)[],
      place: GeoPosition,
      options: ParanOptions & CalcOptions = {},
    ): ParanContact[] {
    ...
    nextCrossing(
      ...
      targetLongitude: number,
      afterJd: number,
      options: CalcOptions & { maxDays?: number; forceSearch?: boolean } = {},
    ): number {
      assertLive();
      const flags = EPHEMERIS_FLAG[options.ephemeris ?? 'swiss'] | (options.flags ?? 0);
      ...
      const calcOptions: CalcOptions = { ...options, flags: options.flags };
      const at = (t: number) => api.calc(t, body, calcOptions);
      const delta = (t: number) => signedDegrees(at(t).longitude - targetLongitude);
      ...
      const step = Math.min(
        Math.max(CROSSING_STEP_DEGREES / meanSpeed, CROSSING_MIN_STEP_DAYS),
        CROSSING_MAX_STEP_DAYS,
      );
      ...
      while (lo - afterJd < maxDays) {
        ...
        if ((dLo < 0) !== (dHi < 0) && Math.abs(dLo - dHi) < 180) {
          ...
          while (b - a > CROSSING_PRECISION_DAYS) {
            ...
            if ((dA < 0) === (dMid < 0)) { a = mid; dA = dMid; } else { b = mid; }
          }
          return (a + b) / 2;
        }
      ...
      throw new SwissEphError(
        `${api.planetName(body)} did not cross ` +
        `${targetLongitude.toFixed(4)}° within ${maxDays.toFixed(0)} days. ` +
        'Pass a larger maxDays to search further.',
        'nextCrossing',
      );
    },
    ...
    solarReturn(
      natalJd: number,
      options: CalcOptions & { after?: number; precessionCorrected?: boolean } = {},
    ) {
    ...
    solarEclipse(
      ...
      if (options.place && options.type !== undefined) {
        throw new SwissEphError(
          'swe_sol_eclipse_when_loc takes no type filter. Passing `type` ' +
          'together with `place` would be silently ignored; filter the result ' +
          'yourself, or drop `place` to search globally.',
          'solarEclipse',
        );
      }
      ...
      if (options.place) {
        ...
        const ret = c.solEclipseWhenLoc(
          afterJd, flags, bufGeo, bufTret, bufAttr, backward, bufError);
        if (ret < 0) {
          throw new SwissEphError(
            readError() ?? 'swe_sol_eclipse_when_loc failed',
      ...
      const ret = c.solEclipseWhenGlob(
        afterJd, flags, options.type ?? 0, bufTret, backward, bufError);
      if (ret < 0) {
        throw new SwissEphError(
          readError() ?? 'swe_sol_eclipse_when_glob failed',
    ...
    lunarEclipse(
      ...
      if (options.place && options.type !== undefined) {
        throw new SwissEphError(
          'swe_lun_eclipse_when_loc takes no type filter; passing `type` for ' +
          'a local search would be silently ignored.',
          'lunarEclipse',
        );
      }
      ...
      const ret = options.place
        ? (writeGeo(options.place), c.lunEclipseWhenLoc(
            afterJd, flags, bufGeo, bufTret, bufAttr, backward, bufError))
        : c.lunEclipseWhen(
            afterJd, flags, options.type ?? 0, bufTret, backward, bufError);
    ...
    heliacal(
      ...
      const ret = c.heliacalUt(
        afterJd, bufGeo, bufDatm, bufDobs, bufObjectName, event, flags,
        bufDret, bufError);

      if (ret < 0) {
        throw new SwissEphError(
          readError() ?? 'swe_heliacal_ut failed', 'swe_heliacal_ut');
      }
     ...
     */
    setSiderealMode(ayanamsa: number, t0 = 0, ayanT0 = 0): void {
      assertLive();
      c.setSidMode(ayanamsa, t0, ayanT0);
    },

    /** The ayanamsa for a moment, in degrees. */
    ayanamsa(jd: number, ephemeris: EphemerisModel = 'swiss'): number {
      assertLive();
      wasm.setValue(bufError, 0, 'i8');
      const ret = c.getAyanamsaExUt(jd, EPHEMERIS_FLAG[ephemeris], bufPosition, bufError);
      if (ret < 0) {
        throw new SwissEphError(
          readError() ?? 'swe_get_ayanamsa_ex_ut failed', 'swe_get_ayanamsa_ex_ut');
      }
      return wasm.getValue(bufPosition, 'double') as number;
    },

    /** Sets the observer's place for topocentric positions, with `Flag.Topocentric`. */
    setTopocentric(longitude: number, latitude: number, altitudeMetres = 0): void {
      assertLive();
    ...
    async loadEphemeris(
      ...
      range: RequiredFilesOptions,
      dir = DEFAULT_EPHE_PATH,
    ): Promise<{ loaded: string[]; missing: string[]; bytes: number }> {
      assertLive();
      ...
      const results = await Promise.all(
        wanted.map(async (name) => [name, await source.read(name)] as const),
      );
     ...
     */
    mountEphemerisDirectory(hostDirectory: string, dir = DEFAULT_EPHE_PATH): void {
      assertLive();
      const NODEFS = (wasm as { NODEFS?: unknown }).NODEFS;
      if (!NODEFS) {
        throw new SwissEphError(
          'NODEFS is unavailable — mountEphemerisDirectory() only works under ' +
          'Node. Use loadEphemeris() in a browser.',
          'mountEphemerisDirectory',
        );
      }
      try {
        wasm.FS.mkdir(dir);
