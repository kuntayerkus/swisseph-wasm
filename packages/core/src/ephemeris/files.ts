...
const PREFIX: Record<EphemerisFileKind, string> = {
 ...
 */
export function ephemerisFileFor(kind: EphemerisFileKind, year: number): string | null {
 ...
 * ```
 */
export function requiredEphemerisFiles(options: RequiredFilesOptions): string[] {
  ...
  if (options.fixedStars) files.push(AUXILIARY_FILES.fixedStars);
  if (options.fictitiousBodies) files.push(AUXILIARY_FILES.fictitiousBodies);
 ...
 */
function sliceOrdinal(year: number): number {
  return year >= 0
    ? Math.floor(year / YEARS_PER_FILE)
    : -Math.ceil(-year / YEARS_PER_FILE);
}

function fileForOrdinal(kind: EphemerisFileKind, ordinal: number): string | null {
  const prefix = PREFIX[kind];
  if (ordinal >= 0) {
