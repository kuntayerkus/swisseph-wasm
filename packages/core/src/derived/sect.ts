 ...
 */
function sectFromAscendant(
  sunLongitude: number,
  ascendant: number,
  twilightAllowance: number,
): { sect: Sect; elevation: number } {
...
export function determineSect(
  sunLongitude: number,
  ascendant: number,
  options: SectOptions & { sunAltitude?: number } = {},
): SectResult {
  ...
  if (method === 'altitude') {
    if (options.sunAltitude === undefined) {
      throw new Error(
        "method:'altitude' needs sunAltitude — compute it with swe.horizontal().",
      );
    }
