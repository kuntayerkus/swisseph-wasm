 ...
 */
export function solarEclipseKind(flags: number): SolarEclipseKind {
  if (flags & EclipseFlag.AnnularTotal) return 'hybrid';
  if (flags & EclipseFlag.Total) return 'total';
  if (flags & EclipseFlag.Annular) return 'annular';
  return 'partial';
}

export function lunarEclipseKind(flags: number): LunarEclipseKind {
  if (flags & EclipseFlag.Total) return 'total';
  if (flags & EclipseFlag.Partial) return 'partial';
  return 'penumbral';
}
 ...
 */
export function optionalTime(value: number): number | undefined {
