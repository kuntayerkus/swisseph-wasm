...
export const Calendar = { Julian: 0, Gregorian: 1 };
/** Verilen takvim tarihinin hangi takvimde okunması gerektiği. */
export function calendarFor(year, month, day) {
...
export function calendarForJulianDay(jd) {
 ...
 */
function wallClockToUtc(year, month, day, hour, minute, second, zone) {
...
export function formatClock(decimalHour) {
 ...
 */
export function resolveTime(input, julday) {
    const dateMatch = /^(-?\d{1,6})-(\d{1,2})-(\d{1,2})$/.exec(input.date.trim());
    if (!dateMatch) {
        throw new Error(`date "${input.date}" was not understood. Expected YYYY-MM-DD ` +
            '(astronomical year numbering for BCE: -0043 = 44 BCE).');
    }
    const timeMatch = /^(\d{1,2}):(\d{2})(?::(\d{2}))?$/.exec(input.time.trim());
    if (!timeMatch) {
        throw new Error(`time "${input.time}" was not understood. Expected HH:MM or HH:MM:SS.`);
    }
    ...
    if (month < 1 || month > 12)
        throw new Error(`Month must be 1-12: ${month}`);
    /*
    ...
    if (day < 1 || day > maxDay) {
        const calendarName = calendarFor(year, month, 1) === Calendar.Julian
            ? 'Julian' : 'Gregorian';
        throw new Error(`"${input.date}" is not a real date: month ${month} of ${year} has ` +
    ...
    let offsetHours;
    let source;
    let alternativeOffsetHours = null;
    ...
    if (POSIX_OFFSET_ZONE.test(zone)) {
        ...
        throw new Error(`"${zone}" uses the inverted POSIX sign convention: it means ` +
    ...
    if (/^(utc|gmt|z)$/i.test(zone)) {
         ...
         */
        if (year < 1583) {
            throw new Error(`An IANA timezone ("${zone}") cannot be used for the year ${year}: ` +
        ...
        try {
            ...
            throw new Error(`timezone "${zone}" was not recognised. Pass an IANA name ` +
