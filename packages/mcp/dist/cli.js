...
function electronConfig(dir, file, platform) {
    if (platform === 'win32') {
        const base = appData();
...
export function knownClients(platform = process.platform) {
    const { command, args } = launchCommand(platform);
    return [
        {
            id: 'claude-desktop',
...
export function renderConfig(client, platform = process.platform) {
    if (client.kind === 'toml')
 ...
 */
export function installIntoJson(client, options = {}) {
    ...
    if (existsSync(file)) {
        ...
        try {
            ...
            return {
                client,
                status: 'failed',
                detail: `${file} is not valid JSON (${error.message}) — ` +
                    'left untouched. Add the block by hand.',
            };
        }
    }
    const map = (document[client.mapKey] ?? {});
...
function configCommand(argv) {
    ...
    if (id) {
        ...
        if (!client) {
            process.stderr.write(`Unknown client "${id}". Known: ${clients.map((c) => c.id).join(', ')}\n`);
            return 1;
        }
    ...
    if (wantJson) {
        process.stdout.write(`${JSON.stringify({ mcpServers: { [SERVER_KEY]: serverEntry() } }, null, 2)}\n`);
        return 0;
    }
    const lines = [];
    for (const client of clients) {
        ...
        if (client.file)
            lines.push(client.file);
        if (client.note)
            lines.push(client.note);
        lines.push('');
...
function installCommand(argv) {
    ...
    if (unknown.length > 0) {
        process.stderr.write(`Unknown client${unknown.length > 1 ? 's' : ''}: ${unknown.join(', ')}\n` +
    ...
    if (targets.length === 0) {
        process.stdout.write('No client config files found on this machine.\n\n' +
    ...
    for (const r of results) {
        ...
        if (r.backup)
            lines.push(`  previous file kept at ${r.backup}`);
        if (r.client.note && r.status === 'written')
            lines.push(`  ${r.client.note}`);
    }
    if (dryRun)
...
async function doctorCommand() {
    const lines = [`swisseph-mcp ${version}`, ''];
    let healthy = true;
    ...
    try {
        ...
        healthy = false;
        lines.push(`✗ the ephemeris failed to load: ${error.message}`);
    }
    lines.push('');
    const { command, args } = launchCommand();
    ...
    if (found.length === 0) {
        ...
        for (const client of found) {
            ...
            if (client.kind === 'json' && client.file) {
                try {
                    const doc = JSON.parse(readFileSync(client.file, 'utf8'));
                    const map = doc[client.mapKey];
...
export async function runCli(argv) {
    ...
    switch (first) {
        ...
        default:
            process.stderr.write(`swisseph-mcp: unrecognised argument "${first}".\n` +
