 ...
 * opportunity.
 */
import { Body, HOUSE_SYSTEM_ALIASES, HOUSE_SYSTEM_NAMES, HouseSystem, evaluateDignities, findAspects, houseOf, normalizeDegrees, MODERN_ORBS, TIGHT_ORBS, TRADITIONAL_MOIETIES, } from '@kuntay/swisseph';
import { explainUnavailable, formatAngle, formatDeclination, formatLongitude, pad, } from './format.js';
...
export function computeChart(swe, jd, place, houseSystem = HouseSystem.Placidus) {
    const positions = [];
    ...
    for (const body of CHART_BODIES) {
        ...
        try {
            ...
            positions.push({
                ...
                unavailable: explainUnavailable(error instanceof Error ? error.message : String(error)),
            });
        }
    }
...
export function aspectPoints(chart, includeAngles = true) {
    const points = chart.positions
 ...
 */
function houseFinder(chart) {
    if (chart.houses.cusps.length !== 12)
        return () => null;
    return (longitude) => houseOf(longitude, chart.houses.cusps);
}
/** `h 7` biçiminde sabit genişlikte ev sütunu. */
const houseColumn = (house) => house === null ? '     ' : `h${String(house).padStart(3)} `;
// --- rendering -----------------------------------------------------------
const NAME_WIDTH = 12;
/**
...
export function renderPositions(chart) {
    ...
    for (const p of chart.positions) {
        ...
        lines.push(`${pad(p.name, NAME_WIDTH)}${retrograde} ` +
...
export function renderHouses(chart) {
    ...
    if (houses.substituted) {
        lines.push(`⚠ The requested house system is undefined at this latitude; Swiss ` +
    ...
    if (gauquelin) {
        lines.push(`These are Gauquelin SECTORS, not houses: ${houses.cusps.length} of them, ` +
 ...
 */
export const ASPECT_ORDER_NOTE = 'Order: strongest first — strength = (1 − orb/allowed) × the aspect\'s ' +
    ...
    'order.';
export function renderAspects(points, schemeName, heading = 'ASPECTS') {
    ...
    for (const a of aspects) {
        ...
        lines.push(`${pad(a.from.name, NAME_WIDTH)} ${pad(a.aspect.name, 13)} ` +
            `${pad(a.to.name, NAME_WIDTH)} orb ${pad(formatAngle(a.orb), 8)}` +
            `(allowed ${formatAngle(a.maxOrb)})${motion}`);
    }
    return lines.join('\n');
}
export function renderDignities(swe, chart) {
    const lines = ['DIGNITIES (essential)'];
    for (const position of chart.positions) {
        if (position.unavailable || position.body === undefined)
            continue;
        if (!DIGNITY_BODIES.includes(position.body))
            continue;
