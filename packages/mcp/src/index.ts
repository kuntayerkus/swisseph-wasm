...
import { z } from 'zod';
import {
  Body, EclipseFlag, HOUSE_SYSTEM_NAMES, HouseSystem, RiseTransit,
  findAspectsBetween, firdariaAt, profection,
...
const dateField = () => z.string().describe(
  'Calendar date as YYYY-MM-DD. Astronomical year numbering, so -0043 is ' +
  '44 BCE. Example: "1990-05-15".');

const timeField = () => z.string().describe(
  'LOCAL clock time at the place, HH:MM or HH:MM:SS. Not UT — pass the time ' +
  'as it was read off a clock there.');

const timezoneField = () => z.string().describe(
  'IANA zone name such as "Europe/Istanbul" — strongly preferred, because it ' +
...
const latitudeField = () => z.number().min(-90).max(90).describe(
  'Geographic latitude in decimal degrees, north positive.');

const longitudeField = () => z.number().min(-180).max(180).describe(
  'Geographic longitude in decimal degrees, EAST positive. Western ' +
  'longitudes are negative: New York is -74.0.');

const placeLabelField = () => z.string().optional().describe(
  'Place name, used only for display.');

const houseSystemField = () => z.string().optional().describe(
  'House system. Either the one-letter code — P=Placidus (default), K=Koch, ' +
 ...
 */
function resolveHouseSystem(input: string | undefined): string {
  if (input === undefined) return HouseSystem.Placidus;
  const raw = input.trim();
  if (raw.length === 0) return HouseSystem.Placidus;

  // Tek karakter: 'i' ile 'I' AYRI sistemler, o yüzden önce birebir bakıyoruz.
  if (raw.length === 1) {
    if (raw in HOUSE_SYSTEM_NAMES) return raw;
    const upper = raw.toUpperCase();
    if (upper in HOUSE_SYSTEM_NAMES) return upper;
  }
  ...
  if (wanted in SPOKEN) return SPOKEN[wanted];

  for (const [code, name] of Object.entries(HOUSE_SYSTEM_NAMES)) {
    if (normalise(name) === wanted) return code;
  }

  throw new Error(
    `house_system "${input}" was not recognised. Pass a one-letter code — ` +
...
const orbSchemeField = () => z.enum(['modern', 'traditional', 'tight']).optional()
  .describe(
    'Which orb scheme to use. "modern" gives the orb to the aspect and is the ' +
...
function header(
  title: string,
  resolved: ReturnType<typeof resolveTime>,
 ...
 */
function altitudeNote(body: string, altitude: number, visible: boolean): string[] {
  const lines = [`Altitude of the ${body} at maximum: ${formatDeclination(altitude)}`];
  if (visible && altitude < 0) {
    lines.push(
      `  ↳ Below the horizon at maximum, yet part of the eclipse is visible ` +
...
server.registerTool('natal_chart', {
  title: 'Natal chart',
  description:
    'Complete chart for one moment and place: planetary positions with ' +
...
}, async ({ date, time, timezone, latitude, longitude, place, house_system, orb_scheme }) =>
  withEphemeris((swe) => {
    const resolved = resolveTime({ date, time, timezone } as TimeInput,
      (y, mo, d, h) => swe.julianDay(y, mo, d, h));
    const location = { latitude, longitude, label: place };
    const chart = computeChart(swe, resolved.julianDay, location,
      resolveHouseSystem(house_system));

    return text(sections(
      header('NATAL CHART', resolved, location, chart.anyMoshier),
      renderSect(chart),
      renderPositions(chart),
      renderHouses(chart),
      renderAspects(aspectPoints(chart), orb_scheme ?? 'modern'),
      renderDignities(swe, chart),
      renderLots(chart),
    ));
  }));
...
server.registerTool('transits', {
  ...
  description:
    'Aspects from the planets at one moment (the transit date) to the ' +
  ...
  const julday = (y: number, mo: number, d: number, h: number) => swe.julianDay(y, mo, d, h);
  ...
  const crossings = findAspectsBetween(
    aspectPoints(transiting, false).map((p) => ({ ...p, name: `t.${p.name}` })),
    aspectPoints(natal, true).map((p) => ({ ...p, name: `n.${p.name}`, speed: 0 })),
    { orbs: scheme },
  );

  const lines = [`TRANSIT ASPECTS (orb scheme: ${scheme.name})`];
  if (crossings.length === 0) lines.push('No transiting aspects within orb.');
  else lines.push(ASPECT_ORDER_NOTE);
  for (const a of crossings) {
    const motion = a.applying === undefined ? '' : a.applying ? '  applying' : '  separating';
    lines.push(
      `${pad(a.from.name, 14)} ${pad(a.aspect.name, 13)} ${pad(a.to.name, 14)}` +
      ` orb ${pad(formatAngle(a.orb), 8)}${motion}`);
  }

  return text(sections(
    header('TRANSITS', transitTime, location, transiting.anyMoshier),
    `Natal: ${natalTime.localLabel} ${natalTime.offsetLabel} → ${natalTime.utcLabel}`,
    renderPositions(transiting),
    lines.join('\n'),
  ));
}));
...
server.registerTool('synastry', {
  ...
  description:
    'Aspects between the planets and angles of two different birth charts. ' +
  ...
  const scheme = ORB_SCHEMES[args.orb_scheme ?? 'modern'];
  const crossings = findAspectsBetween(
    aspectPoints(chartA, true).map((p) => ({ ...p, name: `A.${p.name}`, speed: undefined })),
    aspectPoints(chartB, true).map((p) => ({ ...p, name: `B.${p.name}`, speed: undefined })),
    { orbs: scheme },
  );

  const lines = [`SYNASTRY ASPECTS (orb scheme: ${scheme.name})`];
  if (crossings.length === 0) lines.push('No cross-chart aspects within orb.');
  else lines.push(ASPECT_ORDER_NOTE);
  for (const a of crossings) {
    lines.push(
      `${pad(a.from.name, 14)} ${pad(a.aspect.name, 13)} ${pad(a.to.name, 14)}` +
  ...
  return text(sections(
    'SYNASTRY',
    `A: ${args.a_place ?? ''} ${timeA.localLabel} ${timeA.offsetLabel} → ${timeA.utcLabel}`,
    `B: ${args.b_place ?? ''} ${timeB.localLabel} ${timeB.offsetLabel} → ${timeB.utcLabel}`,
    `Ephemeris: ${ephemerisStatus.description}`,
    'CHART A',
    renderPositions(chartA),
    'CHART B',
    renderPositions(chartB),
    lines.join('\n'),
  ));
}));
...
server.registerTool('return_chart', {
  ...
  description:
    'Finds when the Sun or Moon comes back to its natal longitude, and casts ' +
  ...
  inputSchema: {
    ...
    after_date: z.string().describe(
      'Search forward from this date, YYYY-MM-DD. For "my solar return in ' +
      '2026" pass 2026-01-01.'),
    latitude: latitudeField().describe(
      'Latitude to cast the return chart for — the RELOCATED place if the ' +
  ...
  const location = { latitude: args.latitude, longitude: args.longitude, label: args.place };
  const chart = computeChart(swe, result.jd, location,
    resolveHouseSystem(args.house_system));
  ...
  return text(sections(
    `${args.kind === 'solar' ? 'SOLAR' : 'LUNAR'} RETURN\n` +
    ...
    renderPositions(chart),
    renderHouses(chart),
    renderAspects(aspectPoints(chart), args.orb_scheme ?? 'modern'),
  ));
}));

// --- 5. eclipses ---------------------------------------------------------

const ECLIPSE_TYPES: Record<string, number> = {
  any: 0,
...
server.registerTool('eclipses', {
  title: 'Find eclipses',
  description:
    'The next solar or lunar eclipse after a date, with its phase timings ' +
  ...
  inputSchema: {
    ...
    type: z.enum(['any', 'total', 'annular', 'partial', 'penumbral']).optional()
      .describe(
        'Restrict to one kind. Only valid for a global search — omit it when ' +
  ...
  if ((args.latitude === undefined) !== (args.longitude === undefined)) {
    throw new Error(
      'A local eclipse search needs BOTH latitude and longitude. Only ' +
  ...
  if (local && args.type && args.type !== 'any') {
    throw new Error(
      'A local eclipse search accepts no type filter: Swiss Ephemeris has no ' +
  ...
  const when = (jd: number) => formatJulianDay(jd, (j) => swe.calendarDate(j));
  const options = local
  ...
  if (args.kind === 'solar') {
    ...
    if (eclipse.timings.partialBegin) lines.push(`Partial phase begins: ${when(eclipse.timings.partialBegin)}`);
    if (eclipse.timings.totalityBegin) lines.push(`Totality begins: ${when(eclipse.timings.totalityBegin)}`);
    if (eclipse.timings.totalityEnd) lines.push(`Totality ends: ${when(eclipse.timings.totalityEnd)}`);
    if (eclipse.timings.partialEnd) lines.push(`Partial phase ends: ${when(eclipse.timings.partialEnd)}`);
    if (eclipse.timings.sunrise) lines.push(`Sunrise during the eclipse: ${when(eclipse.timings.sunrise)}`);
    if (eclipse.timings.sunset) lines.push(`Sunset during the eclipse: ${when(eclipse.timings.sunset)}`);
    if (eclipse.local) {
    ...
    if (eclipse.timings.penumbralBegin) lines.push(`Penumbral phase begins: ${when(eclipse.timings.penumbralBegin)}`);
    if (eclipse.timings.partialBegin) lines.push(`Partial phase begins: ${when(eclipse.timings.partialBegin)}`);
    if (eclipse.timings.totalityBegin) lines.push(`Totality begins: ${when(eclipse.timings.totalityBegin)}`);
    if (eclipse.timings.totalityEnd) lines.push(`Totality ends: ${when(eclipse.timings.totalityEnd)}`);
    if (eclipse.timings.partialEnd) lines.push(`Partial phase ends: ${when(eclipse.timings.partialEnd)}`);
    if (eclipse.timings.penumbralEnd) lines.push(`Penumbral phase ends: ${when(eclipse.timings.penumbralEnd)}`);
    if (eclipse.timings.moonrise) lines.push(`Moonrise during the eclipse: ${when(eclipse.timings.moonrise)}`);
    if (eclipse.timings.moonset) lines.push(`Moonset during the eclipse: ${when(eclipse.timings.moonset)}`);
    if (eclipse.local) {
      lines.push('', `From ${args.place ?? 'the given place'} ` +
        `(${Math.abs(args.latitude!).toFixed(4)}°${args.latitude! >= 0 ? 'N' : 'S'} ` +
        `${Math.abs(args.longitude!).toFixed(4)}°${args.longitude! >= 0 ? 'E' : 'W'}): ` +
...
const RISE_BODIES: Record<string, number> = {
...
function planetaryHours(
  ...
  const minutes = (days: number) => `${Math.round(days * 24 * 60)} min`;

  const sequence = (from: number, length: number, offset: number) =>
...
server.registerTool('rise_set', {
  ...
  description:
    'When a body rises, culminates, sets and anticulminates at a place on a ' +
  ...
  return text(sections(
    header('RISE AND SET', start, { ...place, label: args.place }, false),
    lines.join('\n'),
  ));
}));
...
server.registerTool('time_lords', {
  ...
  description:
    'Traditional timing for a date: the annual, monthly and daily profection ' +
  ...
  inputSchema: {
    ...
    year_length: z.enum(['tropical', 'julian', 'egyptian']).optional()
      .describe(
        'Days per year. "tropical" (365.2422) is the default and what modern ' +
  ...
  const location = { latitude: args.latitude, longitude: args.longitude, label: args.place };
  const chart = computeChart(swe, natalTime.julianDay, location,
    resolveHouseSystem(args.house_system));
  ...
  const p = profection(natalTime.julianDay, target.julianDay,
    chart.houses.ascendant, options);
  ...
  return text(sections(
    `TIME LORDS for ${args.target_date}`,
    `Natal: ${natalTime.localLabel} ${natalTime.offsetLabel} → ${natalTime.utcLabel}`,
    `Natal Ascendant: ${formatLongitude(chart.houses.ascendant)}`,
    lines.join('\n'),
  ));
}));
...
server.registerTool('declinations', {
  ...
  description:
    'The declination of each body at a moment, the parallels and ' +
  ...
  const resolved = resolveTime(
    { date: args.date, time: args.time, timezone: args.timezone }, julday);
  ...
  for (const p of points) {
    declinationLines.push(
      `${pad(p.name, 10)} ${formatDeclination(p.declination)}` +
  ...
  if (ties.length === 0) tieLines.push('No parallels or contraparallels within orb.');
  for (const t of ties) {
    tieLines.push(
      `${pad(t.from.name, 10)} ${pad(t.kind === 'parallel' ? 'parallel' : 'contraparallel', 15)}` +
  ...
  if (oob.length === 0) {
    ...
    for (const r of oob) {
      oobLines.push(
        `${pad(r.name, 10)} ${formatDeclination(r.declination)} — past the limit ` +
...
process.stderr.write(
  `swisseph MCP server ready — ephemeris: ${ephemerisStatus.description}\n`);
