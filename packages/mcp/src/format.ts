 ...
 */
export function formatCoordinate(degrees: number, positive: string, negative: string): string {
 ...
 */
export function explainUnavailable(message: string): string {
  const file = MISSING_EPHEMERIS_FILE.exec(message)?.[1];
  if (!file) return message;

  const pkg = SHIPPED_IN.exec(message)?.[1];
  return (
    `the data file '${file}' is not installed where this server runs. ` +
    (pkg
